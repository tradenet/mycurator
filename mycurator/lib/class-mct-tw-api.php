<?php
/**
 * Modified from: Class WordPress Twitter API version 1.0.0
 * at: https://github.com/micc83/Twitter-API-1.1-Client-for-Wordpress
 * 
 */
class mct_tw_Api {
	...
	public function __construct( $args = array() ) {
		...
		//blank key or secret means delete bearer token
                if (empty($this->args['consumer_key']) || empty($this->args['consumer_key'])) {
                    delete_option('mct_ai_twbearer');
                    $this->bearer_token = '';
                    return;
                }
                
		if ( ! $this->bearer_token = get_option( 'mct_ai_twbearer' ) )
	...
	private function get_bearer_token() {
		...
		if ( is_wp_error( $response ) || 200 != $response['response']['code'] )
			return $this->bail( __( 'Can\'t get the bearer token, check your credentials', 'mct_tw_Api' ), $response );
		
		$result = json_decode( $response['body'] );
		
		update_option( 'mct_ai_twbearer', $result->access_token );
	...
	public function query( $query, $query_args = array(), $stop = false ) {
		...
		$transient_name = 'mct_ai_' . md5( $query );
		...
		if ( is_wp_error( $response ) || 200 != $response['response']['code'] ){
			...
			if ( !$stop ){
				...
				return $this->bail( __( 'Bearer Token is good, check your query', 'mct_tw_Api' ), $response );
			}
